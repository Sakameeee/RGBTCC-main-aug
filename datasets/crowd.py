...
class Crowd(data.Dataset):
    def __init__(self, root_path, crop_size=256,
        ...
        # transforms 是 vision 里自带的类，用于数据预处理
        # 首先将图像转换成张量
        # 然后对图像进行归一化处理，指定了RGB通道的均值和标准差
        ...
        self.A_transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize(
                mean=[0.303, 0.243, 0.326],
                std=[0.180, 0.135, 0.148]),
        ])
    ...
    def __getitem__(self, item):
        ...
        a_path = gt_path.replace('GT', 'A').replace('npy', 'jpg')

        RGB = cv2.imread(rgb_path)[..., ::-1].copy()
        T = cv2.imread(t_path)[..., ::-1].copy()
        A = cv2.imread(a_path)[..., ::-1].copy()

        if self.method == 'train':
            keypoints = np.load(gt_path)
            return self.train_transform(RGB, T, A, keypoints)
