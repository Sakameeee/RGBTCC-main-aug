...
class ThermalRGBNet(nn.Module):
    def __init__(self, args=None, embed_dim=512, img_size=224, drop_path=0.):
        ...
        if args:
            ...
            self.aug_backbone.load_state_dict(torch.load(args.pretrained_model), strict=False)
    ...
    def forward(self, inputs):
        ...
        # rgb，t，token 经过多头子注意力处理
        out_fea = self.MSA(rgb_fea1_32, thermal_fea1_32, aug_fea1_32, count_token)
        # 从输出分别提取 rgb 特征值和 token 特征值
        rgb_fea1_32 = out_fea[:, 0:N, :]
        t_token = out_fea[:, N:, :]
        # rgb 是将不同尺度的特征图拼接在一起形成的张量
