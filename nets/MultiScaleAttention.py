...
class Block(nn.Module):
    ...
    def forward(self, rgb, thermal, aug, count_token, i):
        ...
        aug = ProjA(aug)
        RGB_T_A = torch.cat((rgb, thermal, aug), dim=1)
        token_fea = torch.cat((RGB_T_A, count_token), dim=1)
...
class MultiScaleAttention(nn.Module):
    def __init__(self, emb_dim, num_heads, depth=3):
        ...
        self.Proj = nn.ModuleList([
            nn.Linear(emb_dim, int(emb_dim * 10.5)),
            nn.Linear(emb_dim, int(emb_dim * 73.5))
        ])
    ...
    def forward(self, rgb, thermal, aug, count_token):
        B, N, C = rgb.shape
        rgb_ca_t = torch.cat((rgb, thermal, aug), dim=1)
        ca_tokens = torch.cat((rgb_ca_t, count_token), dim=1)
        size = [1, 7, 49]
        feas = []
        for i, block in enumerate(self.MHSA_Block):
            x = block(rgb, thermal, aug, count_token, i)
            if i != 0:
                K = 2 * int(N / size[i])
                x = x[:, 0:K, :]
                count = x[:, -1, :].unsqueeze(1)
                x = self.Proj[i - 1](x)
                x = x.reshape(B, 3*N, C).contiguous()
